...
Extract the shared I2C protocol implementation into a dedicated ESP-IDF component at `packages/robocar/components/i2c-protocol/`. Use the camera version as the source of truth (it had the more complete implementation). Move board-specific configuration (pin numbers, port, frequency) out of the shared header into per-project `i2c_config.h` files.
...
- Added: `packages/robocar/components/i2c-protocol/`
- Modified: `packages/robocar/camera/main/CMakeLists.txt`
- Modified: `packages/robocar/main/main/CMakeLists.txt`
