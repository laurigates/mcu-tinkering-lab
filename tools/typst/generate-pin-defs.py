#!/usr/bin/env python3
"""Generate Typst `#let` bindings from a C pin_config.h.
...
Every binding here derives from the hardware. The firmware version deliberately
does **not** — it used to be read from `version.txt` and emitted as `VERSION`,
which coupled the committed guide's freshness to the release cadence: a
release-please bump changed `version.txt`, which is on none of the drift guard's
trigger paths, so the guard never ran and the committed PDF silently kept
printing the previous version. Regenerating to fix that is itself a `docs:`
commit, which release-please turns into the next release — a loop that never
converges (see issue #439). Keep this file hardware-only.
"""
...
def generate(pin_h_path: Path, out_path: Path) -> None:
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(f"Usage: {sys.argv[0]} <pin_config.h> <output.typ>")
        sys.exit(1)
