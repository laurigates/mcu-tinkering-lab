...
static esp_err_t test_ollama_connectivity(const char* base_url) {
    ...
    s_response_len = 0;
    if (s_response_buffer) s_response_buffer[0] = '\0';
    
    esp_err_t err = esp_http_client_perform(client);
    int status_code = esp_http_client_get_status_code(client);
    
    ESP_LOGI(TAG, "Connectivity test: HTTP Status %d, Response length: %d", status_code, s_response_len);
...
static esp_err_t ollama_init(const ai_config_t* config) {
    ...
    ollama_discovery_config_t discovery_config = {
        .srv_record = OLLAMA_SRV_RECORD,
    ...
    if (discovery_err == ESP_OK) {
        ...
        if (find_err == ESP_OK && num_found > 0) {
            ...
            for (size_t i = 0; i < num_found; i++) {
                ESP_LOGI(TAG, "Service %d: %s:%d (%s) priority=%d weight=%d", 
                         i, services[i].hostname, services[i].port, services[i].ip_addr,
                         services[i].priority, services[i].weight);
            }
            
            // Get the best available service
            ollama_service_info_t best_service;
            esp_err_t best_err = ollama_discovery_get_best_service(services, num_found, &best_service);
            if (best_err == ESP_OK) {
                ESP_LOGI(TAG, "Selected best service: %s:%d", best_service.hostname, best_service.port);
                ...
                if (url_err == ESP_OK) {
                    ...
                    ESP_LOGE(TAG, "Failed to build URL from discovered service: %s", esp_err_to_name(url_err));
                }
            } else {
...
static esp_err_t ollama_analyze_image(const uint8_t* image_data, size_t image_size, ai_response_t* response) {
    ...
    cJSON_AddItemToObject(json, "prompt", cJSON_CreateString(
