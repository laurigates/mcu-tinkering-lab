...
def analyze_energy_conservation():
    ...
    for i in range(100):
        ...
        # Print key points
        if i % 10 == 0:
            print(f"Step {i}: KE={kinetic_energy:.4f}J, RE={rotational_energy:.4f}J, "
    ...
    for i in range(15, len(total_energies)):
        if total_energies[i] < total_energies[i-1] * 0.9:
            ...
            print(f"Drop: {((total_energies[i-1] - total_energies[i])/total_energies[i-1]*100):.1f}%")
            break
    ...
    return times, kinetic_energies, rotational_energies, total_energies, velocities, angular_velocities, motor_powers
