...
def test_robot_context_creation():
    ...
    try:
        ...
        print(f"  📊 Position: ({context.position[0]:.1f}, {context.position[1]:.1f}, {np.degrees(context.position[2]):.1f}°)")
...
async def test_mock_ai_processing():
    ...
    try:
        ...
        for i, mock_response in enumerate(mock_responses):
            ...
            print(f"  ✅ Mock response {i+1}: {command.action} (confidence: {command.confidence:.1f})")
...
def test_integration_with_communication_bridge():
    ...
    try:
        ...
        assert MessageType.AI_COMMAND in bridge.message_handlers, "AI command handler should be registered"
