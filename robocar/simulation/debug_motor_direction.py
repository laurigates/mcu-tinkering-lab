...
def analyze_motor_direction():
    ...
    for i in range(10):
        ...
        print(f"{i:4d} | {voltage:.2f}V   | {back_emf:.4f}V | {motor.current:.3f}A | {motor.torque:.4f} | {robot.state.motor_left_rpm:.1f} | {motor.angular_velocity:.3f}")
        ...
        if i == 0:
            ...
            print(f"  Current derivative: {(voltage - back_emf - motor.resistance * motor.current) / motor.inductance:.6f}A/s")
