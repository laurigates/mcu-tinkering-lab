...
class CameraSimulation:
    ...
    def _setup_camera_intrinsics(self):
        ...
        self.camera_matrix = np.array(
            [[focal_length_px, 0, width / 2], [0, focal_length_px, height / 2], [0, 0, 1]],
            dtype=np.float32,
        )
    ...
    def _create_environment_objects(self) -> dict[str, Any]:
        ...
        objects["walls"] = [
            {
                "type": "plane",
                "position": [room_size / 2, 0, wall_height / 2],
                "size": [0.1, room_size, wall_height],
                "color": [200, 200, 200],
            },  # Right wall
            {
                "type": "plane",
            ...
            },  # Left wall
            {
                ...
                "size": [room_size, 0.1, wall_height],
                "color": [200, 200, 200],
            },  # Front wall
            {
                "type": "plane",
    ...
    def _render_floor(
        self, img: np.ndarray, cam_pos: np.ndarray, cam_forward: np.ndarray, cam_up: np.ndarray
    ):
    ...
    def _screen_to_world_ray(
        self, x: int, y: int, width: int, height: int, cam_forward: np.ndarray, cam_up: np.ndarray
    ) -> np.ndarray:
        ...
        # Ray direction
        ray_dir = cam_forward + nx * tan_half_fov * cam_right + ny * tan_half_fov * cam_up_corrected
    ...
    def start_capture(self, robot_state_callback):
        ...
        self.capture_thread = threading.Thread(
            target=self._capture_loop, args=(robot_state_callback,), daemon=True
        )
