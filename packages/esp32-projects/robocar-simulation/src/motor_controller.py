...
class PIDController:
    ...
    def update(self, setpoint: float, measured_value: float, dt: Optional[float] = None) -> float:
        ...
        self.integral = np.clip(self.integral, -self.config.integral_limit, self.config.integral_limit)
        ...
        if dt > 0:
            ...
            filtered_derivative = (self.config.derivative_filter * raw_derivative + 
...
class EncoderSimulation:
    ...
    def update(self, actual_angular_velocity: float, dt: float):
        ...
        self.velocity = (self.config.velocity_filter * self.velocity + 
    ...
    def update(self, actual_motor_velocity: float, dt: float) -> float:
        ...
        if self.control_mode == ControlMode.OPEN_LOOP:
            ...
            pid_output = self.velocity_pid.update(
                self.velocity_setpoint, measured_velocity, dt
            )
...
def main():
    ...
    for i in range(300):  # 3 seconds
        ...
        if i % 5 == 0:
            ...
            print(f"{t:.2f}\t{pwm_output:.1f}\t{actual_velocity:.2f}\t{measured_velocity:.2f}\t{position:.1f}")
