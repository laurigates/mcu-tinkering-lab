...
esp_err_t motor_controller_init(void)
{
    ...
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << RIGHT_IN1_PIN) | (1ULL << RIGHT_IN2_PIN) | (1ULL << LEFT_IN1_PIN) |
                        (1ULL << LEFT_IN2_PIN) | (1ULL << STBY_PIN),
    ...
    ledc_channel_config_t right_channel = {.speed_mode = PWM_MODE,
    ...
    ledc_channel_config_t left_channel = {.speed_mode = PWM_MODE,
                               ...
                               uint8_t right_direction)
{
    ...
    ESP_LOGD(TAG, "Set individual motors: L=%d/%d R=%d/%d", left_speed, left_direction, right_speed,
             right_direction);
    return ret;
}
