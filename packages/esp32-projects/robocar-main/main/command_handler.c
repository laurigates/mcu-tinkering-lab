...
static const command_entry_t command_table[] = {
    ...
    {"rotate_ccw", CMD_TYPE_MOVE, MOVE_ROTATE_CCW, handle_move_command, 1, "Rotate counter-clockwise"},
...
static esp_err_t handle_led_command(const robot_command_t *cmd) {
    ...
    switch (led_type) {
        ...
        case LED_RGB: {
            rgb_color_t color = {
                .red = cmd->params.param1,
                .green = cmd->params.param2,
                .blue = cmd->params.param3
            };
...
static esp_err_t handle_system_command(const robot_command_t *cmd) {
    if (cmd->subtype == 0) { // reset
        ...
        ESP_LOGI(TAG, "System Info: Commands: %zu, Processed: %lu", 
                command_table_size, handler_state.command_count);
    }
    return ESP_OK;
}
...
esp_err_t command_parse_string(const char *cmd_string, robot_command_t *cmd) {
    ...
    if (param_count < entry->min_params) {
        ESP_LOGW(TAG, "Command %s requires at least %d parameters, got %d", 
                entry->name, entry->min_params, param_count);
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t command_execute(const robot_command_t *cmd) {
    ...
    if (display_is_initialized()) {
        ...
        for (size_t i = 0; i < command_table_size; i++) {
            if (command_table[i].type == cmd->type && 
                command_table[i].subtype == cmd->subtype) {
    ...
    for (size_t i = 0; i < command_table_size; i++) {
        if (command_table[i].type == cmd->type && 
            ...
            ESP_LOGD(TAG, "Executing command: %s from %s", 
                    command_table[i].name, cmd->source);
...
const char* command_type_to_string(command_type_t type) {
    switch (type) {
        ...
        case CMD_TYPE_SOUND: return "SOUND";
        case CMD_TYPE_SERVO: return "SERVO";
        case CMD_TYPE_LED: return "LED";
        case CMD_TYPE_DISPLAY: return "DISPLAY";
        case CMD_TYPE_STATUS: return "STATUS";
        case CMD_TYPE_SYSTEM: return "SYSTEM";
