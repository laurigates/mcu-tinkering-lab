...
static uint16_t angle_to_pulse_us(servo_id_t servo_id, int16_t angle)
{
    ...
    uint16_t pulse_us =
        SERVO_PWM_MIN_PULSE + (uint16_t)(normalized * (SERVO_PWM_MAX_PULSE - SERVO_PWM_MIN_PULSE));
...
static esp_err_t servo_set_pwm(uint8_t channel, uint16_t pulse_us)
{
    ...
    esp_err_t ret = i2c_master_write_to_device(I2C_NUM_1, PCA9685_I2C_ADDR, write_buf,
