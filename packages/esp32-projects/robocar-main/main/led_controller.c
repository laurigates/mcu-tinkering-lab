...
static esp_err_t pca9685_write_reg(uint8_t reg_addr, uint8_t data) {
    ...
    return i2c_master_write_to_device(I2C_NUM_1, PCA9685_I2C_ADDR, 
...
static void blink_timer_callback(TimerHandle_t xTimer) {
    ...
    const rgb_color_t *target_color = blink_state ? &led_state.blink_color : &led_state.blink_off_color;
...
esp_err_t led_blink(led_position_t position, const rgb_color_t *color,
    ...
    if (timer_period < 10) timer_period = 10; // Minimum 10ms
