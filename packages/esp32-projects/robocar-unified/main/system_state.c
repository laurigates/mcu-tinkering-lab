...
#define STATE_LOCK(action)                                                                      \
    do {                                                                                        \
        if (xSemaphoreTake(g_state_manager.state_mutex, pdMS_TO_TICKS(SEMAPHORE_TIMEOUT_MS)) == \
            pdTRUE) {                                                                           \
            action;                                                                             \
    ...
    if (xSemaphoreTake(g_state_manager.state_mutex, pdMS_TO_TICKS(SEMAPHORE_TIMEOUT_MS)) ==
        ...
        if (g_state_manager.ai_state != new_state) {
            ESP_LOGI(TAG, "AI state changed: %s -> %s",
                     ai_state_to_string(g_state_manager.ai_state), ai_state_to_string(new_state));
...
void system_state_update_last_movement(const char *command)
{
    ...
    if (xSemaphoreTake(g_state_manager.state_mutex, pdMS_TO_TICKS(SEMAPHORE_TIMEOUT_MS)) ==
        pdTRUE) {
        strncpy(g_state_manager.last_movement_command, command,
                sizeof(g_state_manager.last_movement_command) - 1);
        g_state_manager.last_movement_command[sizeof(g_state_manager.last_movement_command) - 1] =
            '\0';
        xSemaphoreGive(g_state_manager.state_mutex);
    } else {
        ESP_LOGE(TAG, "State mutex timeout in %s", __func__);
...
void system_state_update_last_sound(const char *command)
{
    ...
    if (xSemaphoreTake(g_state_manager.state_mutex, pdMS_TO_TICKS(SEMAPHORE_TIMEOUT_MS)) ==
        pdTRUE) {
        strncpy(g_state_manager.last_sound_command, command,
                sizeof(g_state_manager.last_sound_command) - 1);
...
const char *system_state_to_string(system_state_t state)
{
    switch (state) {
        case SYSTEM_STATE_INITIALIZING:
            return "INITIALIZING";
        case SYSTEM_STATE_HARDWARE_READY:
            return "HARDWARE_READY";
        case SYSTEM_STATE_NETWORK_CONNECTING:
            return "NETWORK_CONNECTING";
        case SYSTEM_STATE_NETWORK_CONNECTED:
            return "NETWORK_CONNECTED";
        case SYSTEM_STATE_AI_INITIALIZING:
            return "AI_INITIALIZING";
        case SYSTEM_STATE_FULLY_OPERATIONAL:
...
bool system_state_is_operational(void)
{
    ...
    if (xSemaphoreTake(g_state_manager.state_mutex, pdMS_TO_TICKS(SEMAPHORE_TIMEOUT_MS)) ==
        ...
        operational = (g_state_manager.system_state == SYSTEM_STATE_FULLY_OPERATIONAL) &&
                      (g_state_manager.hardware_initialized) && (g_state_manager.camera_available);
        xSemaphoreGive(g_state_manager.state_mutex);
    } else {
        ESP_LOGE(TAG, "State mutex timeout in %s", __func__);
    }
