 ...
 */
typedef struct {
    char *response_text; /**< The response content from the AI backend. Must be freed by the caller
                            using free_response. */
...
typedef struct {
     ...
     * @return ESP_OK on success.
     */
    esp_err_t (*analyze_image)(const uint8_t *image_data, size_t image_size,
