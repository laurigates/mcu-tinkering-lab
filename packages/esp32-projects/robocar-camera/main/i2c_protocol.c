...
_Static_assert(sizeof(sound_data_t) <= I2C_MAX_DATA_LEN, "sound_data_t exceeds I2C_MAX_DATA_LEN");
_Static_assert(sizeof(servo_data_t) <= I2C_MAX_DATA_LEN, "servo_data_t exceeds I2C_MAX_DATA_LEN");
...
void prepare_movement_command(i2c_command_packet_t *packet, movement_command_t movement,
                              uint8_t speed, uint8_t seq_num)
{
    if (!packet)
        return;
...
void prepare_begin_ota_command(i2c_command_packet_t *packet, const char *url, const uint8_t *hash,
                               uint8_t seq_num)
{
    if (!packet || !url)
        return;
