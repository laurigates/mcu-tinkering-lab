...
{
    return addr & RAW_USB_ADDR_EP_MASK;
}
static bool ep_dir_in(uint8_t addr)
{
    return (addr & RAW_USB_ADDR_DIR_IN) != 0;
}
...
#if RAW_USB_DCD_VENDORED
    tusb_rhport_init_t rh_init = {.role = TUSB_ROLE_DEVICE, .speed = TUSB_SPEED_FULL};
                          ...
                          uint8_t quirks)
{
    ...
    if (speed != RAW_USB_SPEED_FULL) {
        ESP_LOGW(TAG, "ESP32-S3 only supports Full Speed; ignoring speed=%u", speed);
    }
    s_desc = *desc;
    s_quirks = quirks;
    memset(s_ep, 0, sizeof(s_ep));
...
#if RAW_USB_DCD_VENDORED
    return dcd_edpt_xfer(USB_RHPORT, ep | RAW_USB_ADDR_DIR_IN, (uint8_t *)data, len) ? ESP_OK
                                                                                     : ESP_FAIL;
#else
...
/* --- Introspection --- */
bool raw_usb_ready(void)
{
    return s_connected && s_enumerated;
}
    ...
    for (;;) {
        ...
        switch (ev.event_id) {
            ...
            case DCD_EVENT_XFER_COMPLETE: {
                ...
                ESP_LOGD(TAG, "EVT xfer ep=%u %s len=%lu", ep, in ? "IN" : "OUT",
                         (unsigned long)ev.xfer_complete.len);
