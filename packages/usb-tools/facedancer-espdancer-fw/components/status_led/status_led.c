 ...
 * DMA is disabled to avoid conflicts with TinyUSB on ESP32-S3 (the same
 * RMT+TinyUSB DMA conflict documented in xbox-switch-bridge ADR-0008):
 *   .flags.with_dma = false
 *   .mem_block_symbols = 48  (ESP32-S3 has 48 symbols per block, not 64)
 *
 * Adapted from xbox-switch-bridge/components/status_led with espdancer modes.
 */
...
esp_err_t status_led_init(void)
{
    ...
    led_strip_rmt_config_t rmt_cfg = {
        ...
        .flags.with_dma = false, /* DMA breaks after TinyUSB init */
    };
...
void status_led_update(void)
{
    ...
    switch (s_mode) {
        case STATUS_LED_BOOT: /* White blink (fast) */
            if (update_blink(now, BLINK_FAST_US)) {
                set_color(LED_BRIGHTNESS, LED_BRIGHTNESS, LED_BRIGHTNESS);
            } else {
                set_color(0, 0, 0);
            }
            break;

        case STATUS_LED_WIFI_AP_UP: /* Cyan solid */
            set_color(0, LED_BRIGHTNESS, LED_BRIGHTNESS);
            break;

        case STATUS_LED_HOST_CONNECTED: /* Green blink (slow) */
        ...
        case STATUS_LED_USB_ACTIVE: /* Blue solid */
            set_color(0, 0, LED_BRIGHTNESS);
            break;

        case STATUS_LED_EMULATING: /* Green solid */
            set_color(0, LED_BRIGHTNESS, 0);
            break;
