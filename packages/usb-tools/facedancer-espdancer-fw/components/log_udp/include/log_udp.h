 ...
 *
 * Starts the ESP32 as a WiFi access point ("espdancer-log"). Connect your
 * Mac/PC to that network, then listen for UDP broadcast:
 *
 *   socat UDP-RECV:4444 STDOUT
 *   nc -u -l 4444
 *
 * The same control-channel SoftAP carries the espdancer RPC (TCP 4444) in
 * Milestone 2; this component only handles the log sink for now.
 *
 ...
 *
 * Adapted from xbox-switch-bridge/components/log_udp (SSID renamed).
 */
#pragma once
 ...
 *
 * Must be called after nvs_flash_init() and before USB init.
 *
