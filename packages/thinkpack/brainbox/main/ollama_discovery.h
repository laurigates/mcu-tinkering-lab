...
/**
 * @brief Convenience function: discover the best Ollama server and return its base URL.
 *
 * Combines ollama_discovery_find_services() + ollama_discovery_get_best_service() +
 * ollama_discovery_build_url() into a single call. The discovery module must already
 * be initialised via ollama_discovery_init().
 *
 * @param url_buffer  Buffer to receive "http://host:port" (no trailing slash, no path).
 * @param buffer_size Size of url_buffer.
 * @return ESP_OK on success, ESP_ERR_NOT_FOUND if no server discovered.
 */
esp_err_t ollama_discovery_find_server(char *url_buffer, size_t buffer_size);
