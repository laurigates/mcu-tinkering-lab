...
static esp_err_t use_fallback_service(ollama_service_info_t *service)
{
    ...
    if (*hostname_end == ':') {
        service->port = (uint16_t)atoi(hostname_end + 1);
    }
...
esp_err_t ollama_discovery_find_server(char *url_buffer, size_t buffer_size)
{
    ...
    esp_err_t err = ollama_discovery_find_services(services, 3, &num_found);
    if (err != ESP_OK || num_found == 0) {
        ESP_LOGW(TAG, "No Ollama services found during discovery");
        return ESP_ERR_NOT_FOUND;
    }

    ollama_service_info_t best;
    err = ollama_discovery_get_best_service(services, num_found, &best);
    if (err != ESP_OK) {
        return err;
    }

    // Build base URL (no trailing path)
    return ollama_discovery_build_url(&best, "", url_buffer, buffer_size);
}
