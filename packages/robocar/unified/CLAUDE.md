...
Four mechanisms, in the order they act on a cycle:
...
The first two gate the *tool*; the last two gate the *line*. The two tool gates are `&&`-ed in `build_request_json()` and answer different questions — rationing alone still narrates a motionless bench, just less often, while change-detection alone would chatter continuously once the robot is driving.
...
A consequence worth stating plainly: **pointed at a genuinely static scene, this robot goes quiet and stays quiet.** That is correct, not broken — which is why `voice` prints the live score and names whichever gate is holding, and why the planner logs `scene: <score>/<threshold>` every cycle.
