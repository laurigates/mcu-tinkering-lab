...
#grid(columns: (1fr, 1fr), column-gutter: 12pt,
  htable(
    (auto, 1fr),
    ([Ch], [Signal]),
    ([#PCA_CH_LED_LEFT_R], [Left LED — R]),
    ([#PCA_CH_LED_LEFT_G], [Left LED — G]),
    ([#PCA_CH_LED_LEFT_B], [Left LED — B]),
    ([#PCA_CH_LED_RIGHT_R], [Right LED — R]),
    ([#PCA_CH_LED_RIGHT_G], [Right LED — G]),
    ([#PCA_CH_LED_RIGHT_B], [Right LED — B]),
    ([#PCA_CH_SERVO_PAN], [Pan servo (SG90)]),
    ([#PCA_CH_SERVO_TILT], [Tilt servo (SG90)]),
    aligns: (center, left),
  ),
  htable(
    (auto, 1fr),
    ([Ch], [Signal]),
    ([#PCA_CH_MOTOR_R_PWM], [Motor R — PWM #sym.arrow.r PWMA]),
    ([#PCA_CH_MOTOR_R_IN2], [Motor R — IN2 (dir) #sym.arrow.r AIN2]),
    ([#PCA_CH_MOTOR_R_IN1], [Motor R — IN1 (dir) #sym.arrow.r AIN1]),
    ([#PCA_CH_MOTOR_L_IN1], [Motor L — IN1 (dir) #sym.arrow.r BIN1]),
    ([#PCA_CH_MOTOR_L_IN2], [Motor L — IN2 (dir) #sym.arrow.r BIN2]),
    ([#PCA_CH_MOTOR_L_PWM], [Motor L — PWM #sym.arrow.r PWMB]),
    ([14–15], [_reserved_]),
    ([], []),
    aligns: (center, left),
  ),
