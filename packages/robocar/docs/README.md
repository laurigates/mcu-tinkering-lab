...
To select and configure the AI backend, edit `packages/robocar/camera/main/config.h`:

1.  **Choose your backend**: Comment out the backend you don't want to use and make sure the one you want is active.
    ```h
    // In packages/robocar/camera/main/config.h

    #define CONFIG_AI_BACKEND_CLAUDE
    // #define CONFIG_AI_BACKEND_OLLAMA
    ```
2.  **Configure API Settings**:
    - For **Claude**: Make sure your `CLAUDE_API_KEY` is set in `packages/robocar/camera/main/credentials.h`.
    - For **Ollama**: Set the `OLLAMA_API_URL` and `OLLAMA_MODEL` in `packages/robocar/camera/main/config.h` to point to your local Ollama server.
